...
struct s* preorder(struct s* node,int num){
    if(node==NULL)
    return NULL;
    if(node->key==num){
        return node;
    }
    struct s* v1= preorder(node->left,num);
    if(v1!=NULL){
        return v1;
    }
...
void csum(struct s* ele,int dist,int sum[]){
    if(ele==NULL||dist<=0)
    return;
    sum[1]=sum[1]+ele->key;
    dist--;
    csum(ele->left,dist,sum);
    csum(ele->right,dist,sum);
}
void psum(struct s* ele,int dist,int sum[],int dia){
    while(dist--&&ele!=NULL){
        sum[1]=sum[1]+ele->key;
        if(dia==1)
        csum(ele->right,dist,sum);
...
int main(){
    ...
    struct s* ele=preorder(root,num);
    //printf("%d\n",ele->key);
    csum(ele,dist+1,sum);
    int dia;
    if(ele->parent!=NULL){
